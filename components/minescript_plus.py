...
from minescript import (set_default_executor, EventQueue, EventType, EntityData, script_loop, render_loop, ItemStack,
                        TargetedBlock,
                        player_inventory, player_get_targeted_block, press_key_bind, screen_name, player_name,
                        player_hand_items,
                        job_info, container_get_items, player_position, entities, echo_json, show_chat_screen,
                        append_chat_history)
...
def version_info():
    print(VersionInfo(**asdict(ver_info()), minescript_plus=_ver, python_version=version, mappings_installed=True),
          file=stderr)
...
class Hud:
    ...
    def add_text(text: str, x: int, y: int, color: tuple = (255, 255, 255), alpha: int = 255, scale: float = 1.0,
                 shadow: bool = False, italic: bool = False, underline: bool = False, strikethrough: bool = False,
                 obfsucated: bool = False, anchorX: float = 0, anchorY: float = 0, justifyX: float = -1,
                 justifyY: float = -1, screens: str | list[str] = "all") -> int:
        """
        ...
        return _add_text(True, text, place_x, place_y, *color, alpha, scale, shadow, italic, underline, strikethrough,
                         obfsucated, anchorX, anchorY, screens)  # type: ignore

    @staticmethod
    def update_text(index: int, text: str, x: int, y: int, color: tuple = (255, 255, 255), alpha: int = 255,
                    scale: float = 1.0,
                    shadow: bool = False, italic: bool = False, underline: bool = False, strikethrough: bool = False,
                    obfsucated: bool = False, anchorX: float = 0, anchorY: float = 0, justifyX: float = -1,
                    justifyY: float = -1, screens: str | list[str] = "all"):
        """
        ...
        place_y = int(y - ((justifyY + 1) * mc.font.lineHeight * scale * 0.5))  # type: ignore
        _update_text(index, True, text, place_x, place_y, *color, alpha, scale, shadow, italic, underline,
                     strikethrough, obfsucated, anchorX, anchorY, screens)
    ...
    @staticmethod
    def add_item(item_id: str, x: int, y: int, count: str = "", scale: float = 1.0, anchorX: float = 0,
                 anchorY: float = 0, justifyX: float = -1, justifyY: float = -1,
                 screens: str | list[str] = "all") -> int:
        """
    ...
    @staticmethod
    def update_item(index: int, item_id: str, x: int, y: int, count: str = "", scale: float = 1.0, anchorX: float = 0,
                    anchorY: float = 0, justifyX: float = -1, justifyY: float = -1, screens: str | list[str] = "all"):
        """
        Adds an item icon to the HUD at the specified position.
        Args:
